...
Solver::Solver(std::vector<int> &ia, std::vector<int>& ja, std::vector<double>& di, std::vector<double>& al, std::vector<double>& au,
    std::vector<double>& b)
{
    N = di.size();
    maxIter = 500;
...
void Solver::BSG_LU()
{
    ...
    double s_Az = 0, p_r = 0, r_r = 0, p_r_prev = 0;
    double a = 0, B = 0, eps2 = 1e-10;
   ...
   // Reverse(U, x0, x0); // x0 = U^(-1)x0
    ...
    for (iter = 1; iter < maxIter + 1 && normR >= eps; iter++)
    {
        Reverse(U, y, z);      // y = U^(-1) * z(k-1)
        A.Ax(y, Ar);           // Ar = A * U^(-1) * z(k-1)
        Direct(L, D, Ar, Ar);  // Ar = L^(-1) * A * U^(-1) * z(k-1)
        s_Az = s * Ar;         // (Ar, s)
        a = p_r / s_Az;

        // x(k) = x(k-1) + a(k) * z(k-1)
        // r(k) = r(k-1) - a(k) * Az(k-1) ("Ar" = A*z(k-1))
        for (int i = 0; i < N; i++)
        {
            x0.vect[i] = x0.vect[i] + z.vect[i] * a;
            r.vect[i] = r.vect[i] - Ar.vect[i] * a;
        ...
        Reverse(L, D, s, y); // y = L^(-T) * s
        A.ATx(y, Ar);        // Ar = A^T * L^(-T) * s
        Direct(U, Ar, Ar);   // Ar = U^(-T) * A^T * L^(-T) * s
        // p(k) = p(k-1) - a(k) * Ar
        for (int i = 0; i < N; i++)
        {
            p.vect[i] = p.vect[i] - Ar.vect[i] * a;
        }
        p_r_prev = p_r;
        p_r = p * r;
        B = p_r / p_r_prev;
...
void Solver::BSG()
{
    ...
    for (iter = 1; iter < maxIter + 1 && normR >= eps; iter++)
    {
        ...
        // x(k) = x(k-1) + a(k) * z(k-1)
        // r(k) = r(k-1) - a(k) * Az(k-1)
        for (int i = 0; i < N; i++)
        {
            x0.vect[i] = x0.vect[i] + z.vect[i] * a;
            r.vect[i] = r.vect[i] - y.vect[i] * a;
        }
        A.ATx(s, y);         // y = AT * s
        // p(k) = p(k-1) - a(k) * ATs(k-1)
        for (int i = 0; i < N; i++)
        {
            p.vect[i] = p.vect[i] - y.vect[i] * a;
        }
        p_r_prev = p_r;
        p_r = p * r;
        B = p_r / p_r_prev;
